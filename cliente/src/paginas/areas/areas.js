...
const Areas = () => { 
  ...
  const agregarRegistro=(event)=>{    
    ...
    Axios.post(`http://${process.env.REACT_APP_SERVER_IP}/areas`,{
      //TODOS LOS CAMPOS
      nombre:nombre,
    }).then((response)=>{
  ...
  const eliminarRegistro=(idarea)=>{
    Axios.delete(`http://${process.env.REACT_APP_SERVER_IP}/areas/${idarea}`).then(()=>{
      obtenerRegistros();
      swal({
        title: "Exito!",
  ...
  const actualizaRegistro=(idarea)=>{
    Axios.put(`http://${process.env.REACT_APP_SERVER_IP}/areas`,{nombre:nuevoNombre,idarea:idarea}).then(()=>{
      obtenerRegistros();
      swal({
        title: "Exito!",
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <div class="modal-body">
                <label for="exampleInputEmail1" class="form-label">Nombre Area:</label>
                <input type="email" class="form-control form-control-sm" placeholder='Ingrese una nueva area' onChange={(event)=>{ setNombre(event.target.value)}}/>        
                { 
                errores.nombre &&
