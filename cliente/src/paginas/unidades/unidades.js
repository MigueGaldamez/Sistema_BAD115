...
const Unidades = () => { 
  ...
  const agregarRegistro=(event)=>{    
    ...
    Axios.post(`${process.env.REACT_APP_SERVER_IP}/unidades`,{
      //TODOS LOS CAMPOS
      nombre:nombre,
      simbolo:simbolo,
    }).then((response)=>{    
  ...
  const eliminarRegistro=(idunidad)=>{
    Axios.delete(`${process.env.REACT_APP_SERVER_IP}/unidades/${idunidad}`).then(()=>{
      obtenerRegistros();
      swal({
        title: "Exito!",
  ...
  const actualizaRegistro=(idunidad)=>{
    Axios.put(`${process.env.REACT_APP_SERVER_IP}/unidades`,{nombre:nuevoNombre,simbolo:nuevoSimbolo,idunidad:idunidad}).then(()=>{
      obtenerRegistros();
