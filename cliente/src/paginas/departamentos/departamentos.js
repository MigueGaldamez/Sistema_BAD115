...
const Departamentos = () => { 
  ...
  const agregarRegistro=(event)=>{    
    ...
    Axios.post(`https://${process.env.REACT_APP_SERVER_IP}/departamentos`,{
      //TODOS LOS CAMPOS
      nombre:nombre,
    }).then((response)=>{
  ...
  const eliminarRegistro=(iddepartamento)=>{
    Axios.delete(`https://${process.env.REACT_APP_SERVER_IP}/departamentos/${iddepartamento}`).then(()=>{
      obtenerRegistros();
      swal({
        title: "Exito!",
  ...
  const actualizaRegistro=(iddepartamento)=>{
    Axios.put(`https://${process.env.REACT_APP_SERVER_IP}/departamentos`,{nombre:nuevoNombre,iddepartamento:iddepartamento}).then(()=>{
      obtenerRegistros();
      swal({
        title: "Exito!",
    ...
    <div class="col container my-4">
      ...
      <div class="mt-4 mb-4">
        ...
        {validarLista.includes(6) &&
          ...
          <DatatableRoles validarLista={validarLista} data={buscar(departamentoLista)} eliminarRegistro={eliminarRegistro} actualizarRegistro={actualizaRegistro} setNuevoNombre={setNuevoNombre}/>
      </div>    
    </div>}
    </>
  );
